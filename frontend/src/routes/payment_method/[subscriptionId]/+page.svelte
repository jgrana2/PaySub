<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			console.error('Se produjo un error al obtener los datos de suscripción:', error);
		}
	});
	...
	function validateCardNumber(event: Event) {
		...
		if (validateCard(creditCardNumber)) {
			...
			target.setCustomValidity('Número de tarjeta inválido');
		}
	...
	async function handleSubmit(event: SubmitEvent & { currentTarget: HTMLFormElement }) {
		...
		if (validateCard(formData.get('card_number')) && isValidCVC(formData.get('cvc'))) {
			...
			try {
				...
				isProcessing = false;
				console.error('Hubo un error al activar la suscripción:', error);
			}
		}
	}
